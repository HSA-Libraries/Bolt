...
#if EXCLUSIVE
#if USE_VECN
            bolt::cl::exclusive_scan( ctrl, input.begin( ), input.end( ), output.begin( ), empty_vecN, fNp );
#else
            bolt::cl::exclusive_scan( ctrl, input.begin( ), input.end( ), output.begin( ) ); // here
#endif
#else
#if USE_VECN
            bolt::cl::inclusive_scan( ctrl, input.begin( ), input.end( ), output.begin( ), fNp );
#else
            bolt::cl::inclusive_scan( ctrl, input.begin( ), input.end( ), output.begin( ) );
#endif
#endif
...
#if USE_VECN
        ::std::partial_sum(refInput.begin(), refInput.end(), refOutput.begin(), fNp);
#else
