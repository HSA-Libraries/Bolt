...
{
	bolt::amp::device_vector<int> A(aSize);
	for (int i=0; i < aSize; i++) {
		A[i] = i+1;
	};

	bolt::amp::count (A.begin(), A.end(), 37);
};
...
void testCount2()
{
	...
	size_t count42 = bolt::amp::count (A, A+aSize, 42);
	size_t count13 = bolt::amp::count (A, A+aSize, 13);

	bolt::amp::control::getDefault().debug(bolt::amp::control::debug::Compile);

	std::cout << "Count42=" << count42 << std::endl;
	std::cout << "Count13=" << count13 << std::endl;
	std::cout << "Count7=" << bolt::amp::count (A, A+aSize, 7) << std::endl;
	std::cout << "Count10=" << bolt::amp::count (A, A+aSize, 10) << std::endl;
};
...
{
	bolt::amp::device_vector<float> A(aSize);
	bolt::amp::device_vector<float> B(aSize);
	for (int i=0; i < aSize; i++) {
		A[i] = static_cast<float> (i+1);
		B[i] = A[i];
	};
	...
	std::cout << "BOLT Count7..15=" << bolt::amp::count_if (B.begin(), B.end(), InRange<float>(7,15)) << std::endl;
}
...
void test_bug(int aSize) 
{
	//int aSize = 1024;
	bolt::amp::device_vector<int> A(aSize);
	bolt::amp::device_vector<int> B(aSize);
	for (int i=0; i < aSize; i++) {
		A[i] = rand() % 10 + 1;
		B[i] = A[i];
	}
	...
	int boltInRangeCount = bolt::amp::count_if (B.begin(), B.end(), InRange<int>(1, 10)) ;
	std:: cout << stdInRangeCount << "   "   << boltInRangeCount << "\n";
	//std::cout << "STD Count7..15=" << std::count_if (A.begin(), A.end(), InRange<float>(7,15)) << std::endl;
	//std::cout << "BOLT Count7..15=" << bolt::amp::count_if (A.begin(), A.end(), InRange<float>(7,15), InRange_CodeString) << std::endl;
}
