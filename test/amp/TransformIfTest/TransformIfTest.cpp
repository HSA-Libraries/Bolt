...
#include "bolt/amp/transform_if.h"
...
struct is_odd
  {
    bool operator()(int x) const restrict(amp, cpu)
    {
      if (x % 2)
		  return true;
	  else
...
//TEST( TransformIfStdVector, OutOfPlaceTransform)
//{
//  int length = 1<<8;
//  std::vector<int> hVectorA( length ), hVectorB( length ), hVectorO( length );
//  std::fill( hVectorA.begin(), hVectorA.end(), 1024 );
...
//
//  //std::transform( hVectorA.begin(), hVectorA.end(), hVectorB.begin(), hVectorO.begin(), std::plus< int >( ) );
//  bolt::amp::transform_if( SVectorA.begin(),
//    SVectorA.end(),
//    SVectorB.begin(),
//    SVectorO.begin(),
//    bolt::amp::plus< int >( ),
...
TEST( TransformIfStdVector, SimpleTest )
{
    ...
    bolt::amp::transform_if( stdInput.begin( ), stdInput.end( ), boltOutput.begin(), bolt::amp::negate<int>(), is_odd() );
...
TEST( TransformIfDVVector, SimpleTest )
{
    ...
    //std::transform(  stdInput.begin( ),  stdInput.end( ), stdOutput.begin(), std::negate<int>() );
    bolt::amp::transform_if( boltInput.begin( ), boltInput.end( ), boltOutput.begin(), bolt::amp::negate<int>(), is_odd() );
	
	//std::vector<int> bOutput(boltOutput.begin(), boltOutput.end());
...
TEST( UnaryTransformIfStdVector, SimpleTest )
{
    ...
    //Serial_transform_if(  stdInput.begin( ),  stdInput.end( ), stdOutput.begin(), std::negate<int>() );
...
TEST( UnaryTransformIfDVVector, SimpleTest )
{
    ...
    //Serial_transform_if(  stdInput.begin( ),  stdInput.end( ), stdOutput.begin(), std::negate<int>() );
    bolt::amp::transform_if( boltInput.begin( ), boltInput.end( ), stencil.begin(), boltOutput.begin(), bolt::amp::negate<int>(), bolt::amp::identity<int>() );
	std::vector<int> bOutput(boltOutput.begin(), boltOutput.end());
...
TEST( BinaryTransformIfStdVector, SimpleTest )
{
    ...
    //std::transform(  stdInput1.begin( ),  stdInput1.end( ), stdInput2.begin(), stdOutput.begin(), std::negate<int>() );
...
TEST( BinaryTransformIfDVVector, SimpleTest )
{
	...
	std::vector<int> bOut(boltOutput.begin(), boltOutput.end());
