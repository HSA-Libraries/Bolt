	...
	namespace cl {
		...
		void transform(const bolt::cl::control &c,  InputIterator first1, InputIterator last1, InputIterator first2, OutputIterator result, 
			BinaryFunction f, const std::string user_code)  {
				...
				detail::transform<T> (c, A, B, Z, f, user_code);
		...
		void transform( InputIterator first1, InputIterator last1, InputIterator first2, OutputIterator result, 
			BinaryFunction f, const std::string user_code)  
		{
			transform(bolt::cl::control::getDefault(), first1, last1, first2, result, f, user_code);
		}
		...
		{
			detail::transform<T>(bolt::cl::control::getDefault(), A, B, Z, f, user_code);
		}
	}//end of cl namespace
...
namespace bolt {
	namespace cl {
		namespace detail {
			...
			template<typename T, typename BinaryFunction> 
			void transform(const control &c, ::cl::Buffer A, ::cl::Buffer B, ::cl::Buffer Z, 
				BinaryFunction f, std::string user_code="")  
			{
