...
public:
    ...
    template<typename DVInputIterator1, typename DVInputIterator2, typename DVOutputIterator, typename BinaryFunction>
    void transform_trf_itr_enqueue( bolt::cl::control &ctl, const DVInputIterator1& first1, const DVInputIterator1& last1,
        const DVInputIterator2& first2, const DVOutputIterator& result,
        const BinaryFunction& f, const std::string& cl_code)
    {
        typedef typename std::iterator_traits<DVInputIterator1>::value_type iType1;
        ...
        *********************************************************************************/
        ...
        kernels[boundsCheck].setArg( 0, first1.getContainer<bolt::cl::device_vector<iType1> >().getBuffer() );
        kernels[boundsCheck].setArg( 1, first1.gpuPayloadSize( ),&first1_payload);
        kernels[boundsCheck].setArg( 2, first2.getContainer<bolt::cl::device_vector<iType2> >().getBuffer() );
        kernels[boundsCheck].setArg( 3, first2.gpuPayloadSize( ),&first2_payload);
        kernels[boundsCheck].setArg( 4, result.getContainer().getBuffer() );
        kernels[boundsCheck].setArg( 5, result.gpuPayloadSize( ),&result_payload);
        kernels[boundsCheck].setArg( 6, distVec );
    ...
    template< typename DVInputIterator, typename DVOutputIterator, typename UnaryFunction >
    void transform_unary_trf_itr_enqueue( ::bolt::cl::control &ctl, const DVInputIterator& first, const DVInputIterator& last,
        const DVOutputIterator& result, const UnaryFunction& f, const std::string& cl_code)
    {
        typedef typename std::iterator_traits<DVInputIterator>::value_type iType;
        typedef typename std::iterator_traits<DVOutputIterator>::value_type oType;
    ...
    */
    ...
    template<typename DVInputIterator1, typename DVInputIterator2, typename DVOutputIterator, typename BinaryFunction>
    void transform_trf_itr_pick_iterator( bolt::cl::control &ctl,const DVInputIterator1& first1,const DVInputIterator1& last1,
        ...
        const std::string& user_code, std::random_access_iterator_tag )
    {
        ...
        else if( runMode == bolt::cl::control::MultiCoreCpu )
        {
            ...
            //TODO - enable bolt TBB code paths			
            //typename bolt::cl::device_vector< iType1 >::pointer firstPtr =  first1.getContainer<bolt::cl::device_vector<iType1> >( ).data( );
            //typename bolt::cl::device_vector< oType >::pointer resPtr =  result.getContainer( ).data( );
            //bolt::btbb::transform(  &firstPtr[ first1.m_Index ], &firstPtr[ last1.m_Index ],
            //                        fancyIter, &resPtr[ result.m_Index ], f );
        ...
        else
        {
            ...
            dblog->CodePathTaken(BOLTLOG::BOLT_TRANSFORM,BOLTLOG::BOLT_OPENCL_GPU,"::Transform::OPENCL_GPU");
            #endif
            device_vector< iType1 > dvInput1( first1.m_it, last1.m_it, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, ctl );
            device_vector< iType1 > dvInput2( first2.m_it, sz, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, true, ctl );
            // Map the output iterator to a device_vector
            device_vector< oType > dvOutput( result,       sz, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, true, ctl );
            //Create transform iterators
            typedef bolt::cl::transform_iterator< DVInputIterator1::unary_func, bolt::cl::device_vector< iType1 >::iterator> trf_itr_1;
            trf_itr_1 trf_begin1(dvInput1.begin(), first1.functor());
            trf_itr_1 trf_end1(dvInput1.end(), last1.functor());
            typedef bolt::cl::transform_iterator< DVInputIterator2::unary_func, bolt::cl::device_vector< iType1 >::iterator> trf_itr_2;
            trf_itr_2 trf_begin2(dvInput2.begin(), first2.functor());

            transform_trf_itr_enqueue( ctl, trf_begin1, trf_end1, trf_begin2, dvOutput.begin(), f, user_code ); 
            dvOutput.data( );
        }
    }
    ...
    void transform_trf_itr_pick_iterator( bolt::cl::control &ctl,const DVInputIterator1& first1,const DVInputIterator1& last1,
        const DVInputIterator2& first2, const DVOutputIterator& result, const BinaryFunction& f,
        const std::string& user_code, bolt::cl::device_vector_tag )
    {
        typedef typename std::iterator_traits< DVInputIterator1 >::value_type iType1;
        typedef typename std::iterator_traits< DVInputIterator2 >::value_type iType2;
        typedef typename std::iterator_traits< DVOutputIterator >::value_type oType;
        ...
        if( runMode == bolt::cl::control::SerialCpu )
        {
            ...
            dblog->CodePathTaken(BOLTLOG::BOLT_TRANSFORM,BOLTLOG::BOLT_SERIAL_CPU,"::Transform::SERIAL_CPU");
            #endif
            typename bolt::cl::device_vector< iType1 >::pointer first1Ptr = first1.getContainer<bolt::cl::device_vector< iType > >( ).data( );
            typename bolt::cl::device_vector< iType2 >::pointer first2Ptr = first2.getContainer<bolt::cl::device_vector< iType > >( ).data( );
            typename bolt::cl::device_vector< oType >::pointer  resPtr    = result.getContainer( ).data( );
            typedef typename bolt::cl::transform_iterator<DVInputIterator1::unary_func, iType1 * > raw_trf_itr1;
            typedef typename bolt::cl::transform_iterator<DVInputIterator2::unary_func, iType2 * > raw_trf_itr2;
            raw_trf_itr1 begin1(first1Ptr.get(), first1.functor() );
            raw_trf_itr1 end1(first1Ptr.get()+sz, first1.functor());
            raw_trf_itr2 begin2(first2Ptr.get(), first2.functor() );

#if defined( _WIN32 )
            //TODO -RAVI need to take care of indexes.
            std::transform( begin1, end1, begin2, stdext::make_checked_array_iterator( resPtr.get(), sz ), f );
#else
            std::transform( begin1, end1, begin2, resPtr.get(), f );
#endif
            return;
        }
        else if( runMode == bolt::cl::control::MultiCoreCpu )
    ...
    */
        ...
        bolt::cl::device_vector_tag )
    {
        ...
        if( runMode == bolt::cl::control::SerialCpu )
        {
            ...
            raw_trf_itr begin(firstPtr.get(), first.functor() );
            raw_trf_itr end(firstPtr.get()+sz, first.functor());

#if defined( _WIN32 )
            //TODO -RAVI need to take care of indexes.
            std::transform( begin, end, stdext::make_checked_array_iterator( resPtr.get(), sz ), f );
#else
    ...
    void transform_detect_random_access( bolt::cl::control& ctl, const InputIterator1& first1,
            const InputIterator1& last1, const InputIterator2& first2, const OutputIterator& result,
            const BinaryFunction& f,const std::string& user_code, bolt::cl::transform_iterator_tag, bolt::cl::transform_iterator_tag )
        {
            transform_trf_itr_pick_iterator( ctl, first1, last1, first2, result, f, user_code,
    ...
    {
        transform_unary_trf_itr_pick_iterator( ctl, first1, last1, result, f, user_code,
            typename  std::iterator_traits< OutputIterator >::iterator_category( ) ); 
    };
