...
int binary_search( iTypePtr1 value, iTypeIter1  a,int start, int length,global comp_function *userFunctor)
{
	int low  = start;
	int high = length;
	while( low < high )
...
)
{
    ...
    if(gx < length1 )
    {
        ...
        pos1 = binary_search(val , input_iter2,input_iter2.m_StartIndex, length2, userFunctor);
    ...
    if(gx < length2 )
    {
        ...
        pos2 = binary_search(val, input_iter1,input_iter1.m_StartIndex,length1,userFunctor);
        riter[pos2 + gx ] = val;
    }
}
